...
import 'package:fisica/views/home/mypage/widgets/footlist.dart';
...
class _Footreport2State extends State<Footreport> with SingleTickerProviderStateMixin {
  ...
  FootData? selectedFoot;

  void initState() {
    super.initState();
    if (!AppStateNotifier.instance.isfootempty) {
      dateList = AppStateNotifier.instance.footdata!.map((data) => data.measuredDate).toList();
  ...
  Future<void> showModalBottomSheetWithStates(BuildContext context) async {
    ...
    await showModalBottomSheet(
      ...
      builder: (context) => ScanData(mode: 'histroy', footdata: selectedFoot!),
    );
  }
  ...
  void _onDateSelected(DateTime newDate) {
    ...
    if (index != -1) {
      setState(() {
        ...
        selectedFoot = AppStateNotifier.instance.footdata!.firstWhere(
