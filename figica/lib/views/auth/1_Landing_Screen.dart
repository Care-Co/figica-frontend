...
class _LandingScreenState extends State<LandingScreen> {
  ...
  Future<void> _handleOnPressed() async {
    ...
    try {
      await UserController.validate(id, _inputType).then((validateData) async {
        print(validateData);
        if (validateData != null) {
          bool expired = validateData.expired;
          ...
          if (expired) {
            ...
            // if (count > 5) {
            //   int additionalMinutes = (count - 5) * 5;
            //   targetTime = targetTime.add(Duration(minutes: additionalMinutes));
            // }
            Duration difference = targetTime.difference(now);
            bool exists = (!difference.isNegative);
            print(exists);
            if (exists) {
              ...
              await DialogManager.showDialogByType(
                context: context,
                dialogType: 'overhome',
                getdescriptionText: SetLocalizations.of(context).getText('rudrh', values: {'minutes': minutes, 'seconds': seconds}),
