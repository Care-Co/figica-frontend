...
class _DeviceScreenState extends State<DeviceScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      //     onPressed: () { print(readblue(bluetoothService[1]));  },
      //     child: null,
      //
      //   ),
      // ),
  ...
  Widget Writeblue(BluetoothService r) {
    // Reads all characteristics

    for (BluetoothCharacteristic c in r.characteristics) {
      if (c.properties.write) {
        c.write(utf8.encode("AT+START"));
  ...
  Future<Widget> readblue(BluetoothService r) async{
    String data = '';
    for (BluetoothCharacteristic c in r.characteristics) {
      if (c.properties.write) {
